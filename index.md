...
This triggers the computation in the browser and returns the result.
(There is also a
[`compute()`](https://r-world-devs.github.io/jsplyr/reference/compute.md)
step under the hood that runs the registered steps; you do not need to
call it yourself —
[`collect()`](https://r-world-devs.github.io/jsplyr/reference/collect.md)
runs it for you.)
...
[`collect()`](https://r-world-devs.github.io/jsplyr/reference/collect.md)
returns a [promise](https://rstudio.github.io/promises/), because the
result is fetched asynchronously from the browser. Reactive outputs such
as `renderDT()` resolve promises for you. In other reactive contexts —
...
[`observe()`](https://rdrr.io/pkg/shiny/man/observe.html) — you must
handle the promise yourself (with
[`promises::then()`](https://rstudio.github.io/promises/reference/then.html)
or the re-exported `%...>%` pipe). See
